...
import { WD, addMinutes, diffMinutes, keyOf, labelFromKey, normalizeTime } from '@/lib/datetime'
...
export function WhenCard({
  ...
  const setDuration = (mins: number) => onEndTime(addMinutes(startTime, mins))
  ...
  return (
    <>
      ...
      <div className={`relative mt-3 ${cardCls}`} style={cardStyle}>
        ...
        {/* starts / ends — typeable 24h HH:MM fields */}
        <div className="grid border-t" style={{ gridTemplateColumns: '1fr 1px 1fr', borderColor: hairline }}>
          ...
          <TimeField label="Ends" value={endTime} onCommit={onEndTime} />
        </div>
        {/* duration */}
