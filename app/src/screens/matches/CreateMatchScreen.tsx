      ...
      <div className="flex h-full flex-col">
            ...
            <div className={`mt-3 flex flex-col gap-[18px] p-[18px] ${cardCls}`} style={cardStyle}>
              ...
              >
                ...
                {/* the invited players, listed under the button so the host can
                    see (and remove) exactly who's been invited. An invite holds
                    no slot, so removing one cancels the pending invite instantly
                    (no confirm) and, on an existing match, expires the real
                    invite row so it stops surfacing on the invitee's side (§5).
                    A player made ineligible by switching to ladies-only is flagged
                    for removal rather than silently kept. */}
                {joinMode === 'invite' && invitedIds.length > 0 && (
                  <div className="mt-2.5 flex flex-col gap-1.5">
                    {invitedIds.map((pid) => {
                      ...
                      const remove = () => {
                        if (existing) {
                          const inv = matchInvites(db, existing.id).find((r) => r.player_id === pid && r.status === 'invited')
                          if (inv) actions.cancelInvite(inv.id)
                        }
                        setInvitedIds(invitedIds.filter((x) => x !== pid))
                      }
                      return (
                        <div
                          key={pid}
        ...
        {showInvite && (
          <InvitePicker
            ...
            onConfirm={(ids) => { setInvitedIds([...invitedIds, ...ids.filter((x) => !invitedIds.includes(x))]); setShowInvite(false) }}
          />
        )}
      </div>
    </Shell>
