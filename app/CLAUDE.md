...
| Formatting | `lib/format.ts` | `hm`/`timeRange` (24h), **`to24h`** (normalise `"HH:mm"` → zero-padded 24h), `dayLabel`, `matchKind`, `skillLabel`, `initials`… |
...
| Types | `lib/types.ts` | `Match`, `User`, `MatchRequest`, `Venue`, `Sport`, `SkillLevel`, statuses |
...
- **Data is mock/in-memory; Auth is wired but dormant.** The store/data layer is still mock/in-memory (`lib/mock/*`, reset on reload except the localStorage keys) — repository accessors and Realtime are not wired. **Auth is the exception:** `lib/supabase.ts` (the shared client) + `context/AuthContext.tsx` use real **Supabase Auth** (`getSession` + `onAuthStateChange`, `signInWithPassword`/`signOut`), exposed via `useAuth()` → `{ user, session, loading, signIn, signInWithMock, signOut }`. It activates only when `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` are set (`.env.local`, gitignored); **until then the client is `null` (`isSupabaseConfigured === false`) and the dev-`mockUser` fallback drives the app** — so today's behaviour is unchanged. When a real session exists it always wins; `AuthContext.toAppUser` maps the auth user → the app `User`, **seeding the trust signals (`matches_played`, `attendance_rate`, …) from `mockUser`** because auth carries only id/email/metadata — replace with a `profiles` fetch when that table lands (TODO in `toAppUser`). `LoginScreen` calls the real `signIn` when configured, else the dev-mock path.
