...
Later stages (do **not** build now): coach booking (10% commission), court booking, player star-ratings, payments, push notifications, geolocation/multi-city, **native iOS + Android apps** (planned), minors/teen support, **social graph / followers + activity feed (Strava-style)** (see §10 — deferred; a lightweight play-history-derived alternative is noted there as a candidate).
...
**Home sections (fixed order):** 1) NEXT UP (imminent joined match) · 2) You're hosting (matches you created) · 3) My Matches (matches you joined but did NOT create) · 4) Matches you saved (bookmarked, not yet joined) · 5) Host CTA. Plus a transient JUST PLAYED card when a finished match awaits post-match input (24h window). The two match buckets — **You're hosting** and **My Matches** — are **never merged**.
- **NEXT UP card lifecycle (don't drop the card at kickoff):** a joined match stays in NEXT UP across **`open → full → live`** — it must **not** disappear when status flips to `live` at `start_time` (NEXT UP membership = open/full/live, not open/full only). Once the match ends it leaves NEXT UP and surfaces as the transient **JUST PLAYED** card for the 24h post-match window. The card's **primary action tracks the timeline** (all read-time from `start_time`/`end_time`, no cron): **pre-start + `live`** → the card just shows the upcoming match (open details / chat) — **no attend button in Stage 1**; **just-played (`completed`, ≤24h) → RECORD RESULTS** (the optional post-match flow — log result → share card, and/or flag a no-show; see §5). Reuse the one canonical card via its `variant` prop; never fork a NEXT UP- or just-played-specific card.
- **Per-section caps & card variant:** You're hosting, My Matches, and Matches you saved each show a **max of 3 matches**. You're hosting uses the **full** card (per-card Edit + Record results); My Matches and Matches you saved use the **brief** card. NEXT UP is unchanged.
- **"See all":** a **"See all" button at the section header inline-start** (top-left in LTR, mirrors in RTL — note the app intentionally overrides this to inline-end per a standing user request) opens that section's full archive. **You're hosting → `/hosting`** (HOSTED archive) and **My Matches → `/my-matches`** (JOINED archive, opens on Upcoming) **always show "See all"** regardless of count — it is the only path to that bucket's `[ Upcoming ] [ Past ]` pills, and **Past matches are never listed on Home**, so gating it on >3 would strand a user's history. **Saved → `/saved`** shows "See all" only when the section holds **more than 3** (its list is flat, no Past pills). See §5 for the saved-matches and display rules.
- **Two separate match archives (hosted vs joined), one layout.** `/hosting` = matches you CREATED; `/my-matches` = matches you JOINED but did NOT create (so a join-only user keeps a full upcoming+past history that never depends on hosting). Both render the **one** `MatchArchiveScreen` with `[ Upcoming ] [ Past ]` pills (Upcoming default) and never merge the buckets; the hosting archive uses the **full** card (per-card Edit), the joined archive the **brief** card. Past is read-only and shows the read-time result chip (Won / Lost / Draw / Cancelled). The **My Matches** section + its See-all is the canonical entry to the joined archive (including for join-only users) — there is **no** separate My Matches link in the Home header.
...
| `/home` | **Home** | Tab. **Default landing.** Sections (capped at 3 each, "See all" when >3): NEXT UP · You're hosting (full card) · My Matches (brief card; joined, not hosted) · Matches you saved (brief card; bookmarked, not joined) · Host CTA (+ transient JUST PLAYED). Header actions = **+** (create match) and the avatar/gear (Profile + Settings) **only** — no calendar/My-Matches link. Pending **invites surface as a transient pop-up sheet over Home** on load (see §5 invite flow); accepted invites land in NEXT UP/My Matches |
...
| `/my-matches` | My Matches (JOINED archive) | Sub-screen of Home. Matches you joined but did NOT create. `[ Upcoming ] [ Past ]` pills (Upcoming default); brief card; Past read-only with result chip. Home "My Matches" See-all lands here (the canonical entry — no Home-header link) |
| `/hosting` | You're hosting (HOSTED archive) | Sub-screen of Home. Matches you created. Same `[ Upcoming ] [ Past ]` pills (Upcoming default); full card (per-card Edit); Past read-only with result chip. Home "You're hosting" See-all lands here |
...
| `/matches/:id/results` | Post-match | Sub-screen, **all optional**: log result (win/lose/draw → share card) and/or flag a no-show. Open ≤24h after end |
...
- **Invite flow (full UX — host-initiated):**
  ...
  - **Invited player — Home pop-up:** on `/home` load, any still-joinable `invited` rows surface as a **transient invitation sheet/modal over Home** (not a route/tab), built from the canonical match card. Three actions:
    - **Accept** → insert `match_players`, set invite `joined`, decrement `spots_available`, **add player to the chat thread and route them into `/chat/:threadId`**; the match now appears on the player's Home under **NEXT UP / My Matches** (joined-match sections), never "You're hosting". Post system message "[Name] joined". **The acceptance propagates live (Supabase Realtime) to every surface on both sides — no manual refresh:** the player's name + avatar fill the roster on the canonical match card and Match Details, the joined count / `spots_available` updates everywhere the card renders, the chat thread's member roster gains the player, and the **host's Match Details shows the invitee as `Accepted`/joined** (invite row no longer pending).
...
**Match status lifecycle:** `open → full → live → completed → closed` (+ `cancelled`). **Time-based transitions are computed from `start_time`/`end_time` at read time — no cron jobs.** `completed` = ended, within 24h post-match window; `closed` = 24h passed, **no-show-flagging window shuts** (results + share cards stay open anytime).
...
**Host departure — NO auto-reassignment of the host role.** The host role is **pre-match only** — approve/decline requests, invite, edit, cancel all become moot once the match starts. So when a host **no-shows**, the system does **not** promote or auto-assign a new host — there is nothing left for a successor to do. The host just gets the no-show mark like anyone (within-2h cancel = auto-mark; otherwise any player who showed flags them — see no-show rules); the **result stays first-submitter** (any participant logs it); the **group chat persists unchanged**. Do **not** build a host-succession / "next-in-line" mechanism in Stage 1 — it hands unrequested authority to a random player and spawns edge cases (who's next, they also didn't show, they don't want it) for no real benefit. The app itself is the host of the logistics (match object + roster + chat); it needs no human successor.
- **The one genuinely different case — host cancels/bails *before* start (the court was in their name):** the match can be stranded. **Stage 1 handling = the existing ≥2h-cancel flow notifies all joined players (match cancelled).** Auto-assignment wouldn't help anyway (the booking was the host's). **Deferred (Stage 1.5+, confirm before building):** optionally let a remaining joined player **volunteer** to take over an about-to-be-orphaned match ("host cancelled — take over this match?") — opt-in, never system-forced. Not Stage 1.

**No-show = reputation only. No automatic blocking anywhere.** A no-show is a visible mark on the profile; no block, no penalty. **Stage 1 keeps this deliberately light** — no attend button, no provisional drafting, no required host review (that's engagement we'd only be assuming; revisit once real usage shows flaking is an actual problem). Default: everyone who joined is treated as **played**. A no-show lands only these ways:
- **≥2h cancellation = safe** for anyone (host or player) — no mark.
- **Within-2h cancellation → auto no-show** (objective timestamp; immediate; no UI; host and players alike).
- **Optional "didn't show" flag** for the rare ghost: the **host can flag a player**, and **any player who showed can flag the host** (players cannot flag each other). One flag lands it (reputation-only, low-stakes); the flagged person is **notified** and may **dispute** → surfaces as `contested` until resolved.
- **Public attendance % + no-show count** on the profile do the rest — a passive deterrent that needs no logging.
- No corroboration thresholds, no attend check-in, no auto-block rules — all considered and cut for Stage 1 simplicity.

**Post-match (all optional in Stage 1 — never blocks the core loop):** the match just completes and everyone who joined defaults to **played**. Two optional actions, surfaced on the transient **JUST PLAYED** card (and later a push, §1.10): (1) **log the result** (win/lose/draw) → canonical immediately (**first-submitter**, no gate), posts to Match Details + the match chat, and **triggers the share card**; editable **anytime** by any participant or the **host as scorekeeper** (fixes a wrong score); (2) **flag a no-show** if someone ghosted (host→player, player→host — see no-show rules). **No required "who played" step, no star-rating, no points/ranking** (results stay low-stakes — that's what lets one tap set them; if points/ranking ever arrive they flip to high-stakes, revisit then — see §10). If nobody acts, the match counts as played-by-all with no result — no stuck state. **Logging a result and sharing a card are available anytime** (per-player — each participant makes their own); first-submitter sets the canonical score but **never closes/locks the match**, and results/cards have **no deadline**. The **only** time-bounded post-match action is the **no-show flag** — it must be raised **within ~24h of `end_time`** (the post-match window) so a stale grudge can't land a reputation mark days later; the objective within-2h-cancel mark is unaffected. So `closed` (24h) now means only that the **no-show-flagging window** has shut — results and share cards stay open.

**Share card (post-match poster):** auto-offered when a result is logged, and also generatable on demand. The sharer picks one of **three display variants**: (a) **result + score** (e.g. "Won 6–4 6–2"); (b) **win only** (outcome, no score); (c) **match details only** (sport · venue · date · roster, **no result** — works even when nobody logged a result: a pure "I played / come join" acquisition card). Brand-styled, Western/tabular numerals, one-tap share (Web Share API), and it carries a **join QR/link** so a share can convert (growth loop, §10). Always optional and player-initiated.
...
**Discover excludes matches you're already in (dedup → Home):** a match the user has **joined**, has a **pending request** on (approval mode), or **is hosting** drops out of that user's Discover feed and instead lives on their **Home** — joined → NEXT UP / My Matches; hosting → You're hosting; **pending request → shown with a "Requested" state** (default: in My Matches by timing, badge `--color-warning`-adjacent, until approved/declined). Invite matches are private offers and never list in Discover anyway. **Saved (bookmarked) matches STAY in Discover** (save ≠ join; still joinable, bookmark icon filled). Net: Discover only surfaces matches the user can still freshly act on; once they act, it moves to Home. The feed stays seeded for everyone regardless.

**Home section display (Stage 1 rule):** NEXT UP unchanged. **You're hosting**, **My Matches**, **Matches you saved** each render at most **3** cards; a **"See all"** at the section header inline-start (app intentionally renders it inline-end). **You're hosting and My Matches always show "See all"** (the only path to their Upcoming/Past archive — Past is never on Home); **Matches you saved shows it only when >3**. Card variant: You're hosting = **full** card; My Matches & Matches you saved = **brief** card. All variants come from the one canonical match card via a `variant` prop — never per-section components. The You're hosting and My Matches buckets are **never merged**.

**Matches you saved (saved ≠ joined):** a user can **bookmark a match they haven't joined** via a save toggle on the canonical card. Saving is a personal bookmark only — it does **not** join, request, or hold a slot (consistent with the no-slot-hold rule). A match leaves the Saved section when the user **joins** it (moves to NEXT UP/My Matches), **un-saves** it, or it's **no longer joinable** (full/cancelled/started — read-time, no cron). Saved state reflects **live** on the card's bookmark icon everywhere it renders (Discover, Home, Match Details) via Realtime / shared store. Backed by `saved_matches` (see §6).
...
- `match_players` — id, match_id, player_id, joined_at, **attended** (default = played; set `false` by a within-2h cancel (auto) or a landed no-show flag). No attend-tap in Stage 1.
...
- `no_show_flags` — id, match_id, subject_player, set_by, status (`confirmed`/`contested`), created_at, **unique(match_id, subject_player)**. The optional "didn't show" flag: the **host flags a player**; **any player who showed flags the host** (no peer-vs-peer). One flag lands it (reputation-only); the flagged person is notified and may dispute → `contested`. **Flags accepted only within ~24h of `end_time`** (read-time guard against stale grudges) — results + share cards have no such window. No corroboration threshold, no attend-tap, no provisional/expiry. RLS: host writes for their own match; a present player may write only against the host.
- `match_results` — id, match_id, player_id, result (win/loss/draw) — optional, feeds win rate. **First submission sets the match result (no 2-report gate); any participant or the host can edit anytime** (host = scorekeeper) — no deadline, never closes the match. Share card reads from here.
...
## 10. Growth / go-to-market & deferred social features (reference — NOT Stage 1 build scope)

Captured 2026-06-19. These are growth/GTM notes plus one deferred social feature — **not** build tasks; core build scope (§1–§8) is unchanged. Complements the Stage 1 marketing plan. Guiding principle: **free player-matching is the acquisition engine, not the thing you charge for** — monetization (coach/court, §1 later stages) rides on *liquidity*, never on charging players. So growth = **manufacture supply (matches) first, then ride the density of groups that already exist.**

**North-star metric:** matches **filled and played per week** + repeat players — *not* signups. A new user's first match *must* fill or they churn; concierge the first ~50 by hand.

**GTM tactics (reference, not features):**
- **Seed supply yourself.** Run real recurring games ("Connect! Padel Friday") so the feed holds genuine joinable matches, not a ghost town.
- **Recruit organizers, not individuals.** Win the *admin* of existing padel/tennis WhatsApp groups and migrate their weekly game into the app (one organizer ≈ 20–30 players + recurring supply). Highest-leverage channel.
- **Flip court-booking into a partnership.** Fill clubs' empty off-peak slots in exchange for promotion / QR posters / cheaper rates — the venue carries the cost *and* the distribution (vs paying to fill courts yourself).
- **Captive communities.** Residential compounds + big employers (QatarEnergy, banks, hotels) have courts + group chats; a private "compound/company match" pocket feels alive instantly.
- **Amplifiers — throttle until the feed is alive:** IG build-in-public, **Snapchat geofilters at courts** (Gulf-specific), micro-influencers, local press, QR posters.
- **Narrow the beachhead:** padel + **2–3 clubs first** — density beats spread. Don't run every tactic at once (solo builder).

**Product-native growth loops (these have build hooks — tag Stage-1-lite / post-launch; confirm before building):**
- **"Need a 4th? Send link"** — one-tap invite to fill a spot; the half-full match *is* the core viral mechanic (ties to the invite flow, §5).
- **Post-match share card (Strava-style)** — auto-generated, brand-styled, with a **join QR/link so a story becomes a signup**, not just a flex. One-tap share (Web Share API). **3 display variants — result+score / win-only / match-details-only**; the details-only variant needs no result, so it doubles as a pure acquisition card (full spec in §5). (Already the 1.13 "social share card" grow idea.)
- **Referral = status, not cash** (free app): badges, top-inviter leaderboard, early Stage-2 access. (1.13.)
- **Strongest FOMO for this app = match scarcity** ("a game you want is 3/4 full and someone you've played with is in it") — lean here before any social-graph vanity.

**Competitive mode / points / ranking — DEFERRED (not Stage 1):** Stage-1 results are win/lose/draw, "just for fun," **low-stakes** — which is *why* one report can set the result and the share card can fire instantly (§5). Adding points/ranking would flip results to **high-stakes** (incentive to cheat) and require the rigor we deliberately skip: host-referee override on results, stricter/confirmed result entry, dispute handling. Park until post-launch; don't let it widen Stage 1. (Note: the host *can* already correct **attendance/no-shows** on their match per §5 — that's reputation accuracy, distinct from a competitive points engine.)

**Connections / social graph — DECISION:**
- **DEFERRED (not Stage 1):** full Strava-style **follower system + activity feed**. Why: (a) it's empty at cold-start too — it *amplifies* an active network, it doesn't *create* one (same trap as launching coaches/venues early); (b) follower counts invite vanity/status games and look "sad" when low on a young app, cutting against the **"profiles = accountability/trust, not clout"** stance (§5 — profiles are public-by-default *as a trust mechanism*). Listed in §1 deferred features.
- **LIGHTWEIGHT CANDIDATE (under consideration — confirm before building):** derive an *implicit* graph from **play history (`match_players`)** instead of an explicit follow system. Three cheap surfaces: (1) **"You've played with N of these players"** social-proof line on the canonical card / Match Details; (2) a notification **"someone you've played with is hosting"**; (3) a post-match **"add as a regular"** — a one-way favorite (no follower count) so you see when partners next play. If adopted it touches the **canonical card + profile + notifications** and would move into §5. Delivers most of the FOMO with near-zero new surface and no empty-feed/vanity problem.
