...
- **NEXT UP card lifecycle (don't drop the card at kickoff):** a joined match stays in NEXT UP across **`open → full → live`** — it must **not** disappear when status flips to `live` at `start_time` (NEXT UP membership = open/full/live, not open/full only). Once the match ends it leaves NEXT UP and surfaces as the transient **JUST PLAYED** card for the 24h post-match window. The card's **primary action tracks the timeline** (all read-time from `start_time`/`end_time`, no cron): **pre-start (~30 min before) + `live` → ATTEND** ("I'm here", pre-fills who-played per §5); **just-played (`completed`, ≤24h) → RECORD RESULTS** (the 2-step post-match flow). **No standalone attend button in the just-played state** — it would duplicate the "who played?" step, which is already pre-filled from the check-ins. Reuse the one canonical card via its `variant` prop; never fork a NEXT UP- or just-played-specific card.
...
**Attend check-in (NEXT UP):** the **attend** button is a low-friction *positive presence signal*, **not** a verdict on no-show. It does **not** auto-decide attendance — tapping it can't prove presence (a no-show can tap from home) and not-tapping can't prove absence (honest players forget / are mid-game), so it never lands a no-show on its own. **Display window is read-time computed from `start_time`** (no cron): opens ~30 min before start and stays open through the `live` + just-played window — never visible days ahead in NEXT UP, since an early tap is meaningless. A tap **pre-fills** the player as "Played" in the post-match "who played?" step. The no-show corroboration path (below) is still the source of truth for the cases where the signal and reality diverge.

**Post-match (2 steps only in Stage 1):** 1) Who played? (everyone defaults to "Played"; **attend check-ins pre-fill this** — confirm/flag exceptions only; tap to flag a no-show) · 2) Result win/lose/draw (optional, "just for fun", feeds win rate). **attend covers only the attendance half; it carries no score, so step 2 is still required and is the only source of win rate.** **No star-rating in Stage 1.** Auto-closes once 2 players report results, or after 24h. Result is canonical on Match Details and also posts inline in the match chat.
