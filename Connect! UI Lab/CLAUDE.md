...
**Discover excludes matches you're already in (dedup → Home):** a match the user has **joined**, has a **pending request** on (approval mode), or **is hosting** drops out of that user's Discover feed and instead lives on their **Home** — joined → NEXT UP / This week; hosting → You're hosting; **pending request → shown with a "Requested" state** (default: in This week by timing, badge `--color-warning`-adjacent, until approved/declined). Invite matches are private offers and never list in Discover anyway. **Saved (bookmarked) matches STAY in Discover** (save ≠ join; still joinable, bookmark icon filled). Net: Discover only surfaces matches the user can still freshly act on; once they act, it moves to Home. The feed stays seeded for everyone regardless.
